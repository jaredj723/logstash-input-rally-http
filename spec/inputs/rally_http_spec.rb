...
require 'logstash/inputs/rally_http'
require 'flores/random'

describe LogStash::Inputs::Rally_HTTP do
  let(:metadata_target) { "_rally_http_metadata" }
  let(:queue) { Queue.new }
  ...
  let(:klass) { LogStash::Inputs::Rally_HTTP }
  ...
  describe "events" do
    ...
    shared_examples "unprocessable_requests" do
      let(:poller) { LogStash::Inputs::Rally_HTTP.new(settings) }
      subject(:event) {
